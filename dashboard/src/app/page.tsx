import { listDatabases, listProjects } from "@/lib/dokploy";
...
export default async function Page() {
  try {
    const [databases, projects] = await Promise.all([listDatabases(), listProjects()]);
    return <DatabasesView databases={databases} projects={projects} />;
  } catch (e) {
      ...
      <div className="mx-auto max-w-2xl px-6 py-24">
        <div className="rounded-2xl border border-[var(--color-danger)]/40 bg-[var(--color-danger-soft)] p-6">
          ...
          <p className="mt-2 text-sm text-[var(--color-fg-muted)]">
            The dashboard couldn&apos;t talk to the Dokploy API. Check that Dokploy is running and
            that <code className="font-mono">DOKPLOY_URL</code>,{" "}
            <code className="font-mono">DOKPLOY_EMAIL</code> and{" "}
            <code className="font-mono">DOKPLOY_PASSWORD</code> in{" "}
